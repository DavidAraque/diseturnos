...
if ($_POST['accion'] == "ListarModulos") {
    ...
    if (!empty($arrServicio)) {
        for ($i = 0; $i < count($arrServicio); $i++) {
            ...
            $eliminar = '<button class="btn btn-icon btn-sm btn-danger" onClick="eliminarModulo(' . $arrServicio[$i]['id_modulo'] . ')" title="Eliminar Modulo"><i class="bx bx-trash"></i></button>';
