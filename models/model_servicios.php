...
if ($_POST['accion'] == "ListarServicios") {
    ...
    $query = mysqli_query($mysqli, "SELECT s.id,s.nombre_servicio,s.letra_servicio,s.estado FROM db_servicios s");
    $datos = array();
...
if ($_POST['accion'] == 'ObtenerServicio') {
    ...
    require_once('../config/conexion.php');
    try {
        $stmt = $mysqli->prepare("SELECT s.id,s.nombre_servicio,s.letra_servicio FROM db_servicios s WHERE s.id = ?");
        $stmt->bind_param('s', $id_servicio);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($id, $nombre_servicio,$letra_servicio);
        $stmt->fetch();
        if ($id > 0) {
            $respuesta = array(
                'codigo' => 0,
...
if ($_POST['accion'] == 'RegistroServicio') {
    ...
    if (empty($nombreservicio) || empty($letraservicio)) {
        $respuesta = array(
            "codigo" => 2,
        ...
        try {
            ...
            $ejecutar = $mysqli->prepare("INSERT INTO db_servicios (nombre_servicio,letra_servicio,estado,fecha_registro) VALUES (?,?,'A',NOW())");
            $ejecutar->bind_param("ss", $nombreservicio,$letraservicio);
            $ejecutar->execute();
...
if ($_POST['accion'] == 'ActualizarServicio') {
    ...
    if (empty($idunicodelservicio) || empty($nombreservicio) || empty($letraservicio)) {
        $respuesta = array(
            "codigo" => 2,
            "respuesta" => 'Verificar los Campos Vacios',
        );
        ...
        require_once('../config/conexion.php');
        try {
            $ejecutar1 = $mysqli->prepare("UPDATE db_servicios  SET  nombre_servicio = ?, letra_servicio = ? WHERE id = ?");
            $ejecutar1->bind_param("sssss", $nombreservicio,$letraservicio, $idunicodelservicio);
            $ejecutar1->execute();
